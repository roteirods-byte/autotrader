...
def now_date(): return datetime.datetime.now(TZ).strftime("%Y-%m-%d")
def now_time(): return datetime.datetime.now(TZ).strftime("%H:%M:%S")
def apply_theme():
    css = f"""
    <style>
      ...
      [data-testid="stTextInput"] input {{ max-width: {SIZES['INPUT_W']}px; }}
    </style>
    """
