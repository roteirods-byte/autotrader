# bloco_30_entrada — Swing (4H) e Posicional (1D)
import pandas as pd, streamlit as st
from planilhas.commons.bloco_00_tokens import COINS
def _table_base():
    return pd.DataFrame({"Par":[""],"Sinal":["LONG"],"Preço":[0.0],"Alvo":[0.0],
                         "Ganho %":[0.00],"Assertividade %":[0.00],"Data":[""],"Hora":[""]})
def render_entrada_panel():
    ...
    with colA:
        ...
        dfA = _table_base()
        st.data_editor(dfA, key="entrada_swing",
            column_config={
    ...
    with colB:
        ...
        dfB = _table_base()
        st.data_editor(dfB, key="entrada_posicional",
            column_config={
                "Par": st.column_config.SelectboxColumn("Par", options=COINS),
                "Sinal": st.column_config.SelectboxColumn("Sinal", options=["LONG","SHORT"]),
                "Preço": st.column_config.NumberColumn("Preço", format="%.3f"),
