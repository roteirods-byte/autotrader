import os, re, uuid, streamlit as st
...
def render():
    ...
    st.markdown("""
    <style>
      div[data-testid="stAppViewContainer"] .main .block-container{ max-width:1306px; padding-top:1.5rem; }
    ...
    # Prefixo único para as keys (previne DuplicateWidgetID se houver 2 chamadas acidentais)
    prefix = "email_" + uuid.uuid4().hex[:6] + "_"
    ...
    with st.container(border=True):
        ...
        with c1:
            mail_user = st.text_input("E-mail (remetente)", value=d["mail_user"],
                                      key=prefix+"from", help="Seu Gmail que envia")
        with c2:
            app_pwd = st.text_input("Senha do aplicativo (Gmail)", type="password",
                                    key=prefix+"pwd", help="Senha de App de 16 dígitos")
        with c3:
            mail_to = st.text_input("Enviar para", value=d["mail_to"],
                                    key=prefix+"to", help="Destino dos alertas")
        with c4:
            acao = st.button("TESTAR/SALVAR", key=prefix+"btn")
