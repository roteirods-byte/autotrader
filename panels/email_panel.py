...
def _smtp_send(gmail_user: str, app_password: str, to_addr: str) -> tuple[bool, str]:
    """Tenta serviço interno; se não houver, usa SMTP Gmail direto."""
    try:
...
def _load_defaults() -> dict:
    """Fallback inicial a partir das variáveis de ambiente do Render."""
    return {
...
def render():
    # CSS DO LAYOUT (sempre dentro do render)
    st.markdown(
        """
        <style>
          ...
          /* gap entre colunas da primeira linha */
    ...
    # Painel com borda (oficial Streamlit)
    with st.container(border=True):
        ...
        with c1:
            mail_user = st.text_input(
                "E-mail (remetente)",
                value=defaults["mail_user"],
                key="email_from_input",
                help="Seu Gmail que envia",
            )
        with c2:
            app_pwd = st.text_input(
                "Senha do aplicativo (Gmail)",
                type="password",
                key="email_app_pwd_input",
                help="Senha de App de 16 dígitos",
            )
        with c3:
            mail_to = st.text_input(
                "Enviar para",
                value=defaults["mail_to"],
                key="email_to_input",
                help="Destino dos alertas",
            )
        with c4:
            acao = st.button("TESTAR/SALVAR", key="email_test_save_btn")
        ...
        if ok:
            ...
            try:
                ...
                with get_session() as s:
                    s.add(EmailSetting(mail_from=mail_user, mail_to=mail_to, mail_user=mail_user))
                    s.commit()
            except Exception:
                pass
        else:
